...
# Razorwind - Color Variants Plugin

**Razorwind - Color Variants** is a package that can create alternate color variants from design tokens. The following color variants are supported:
...
```bash
pnpm add -D @razorwind/color-variants
```
...
```bash
npm install -D @razorwind/color-variants
```
...
```bash
yarn add -D @razorwind/color-variants
```
...
export default defineConfig({
  plugins: [
    colorVariants(),
    // colorVariants(["dimmed", "high-contrast", "protanopia"])
    // colorVariants({ variants: ["dimmed", "high-contrast"] })
  ]
});
```
...
| ---------- | ----------------------------- | -------------------------------------------------------- |
| `variants` | `["dimmed", "high-contrast"]` | Color variants to generate from each extracted token set |

Supported variants: `dimmed`, `high-contrast`, `protanopia`, `deuteranopia`, `tritanopia`, `achromatopsia`, `monochromatic`.

When tokens are a multi-theme record (`{ dark, light }`), each variant is appended to the theme key (`darkHighContrast`, `lightDimmed`, …). When tokens are a single tree, the original set is stored as `default` and variants use camelCase keys (`highContrast`, `dimmed`, …).

## Development

### Building

Run `nx build color-variants` to build the library.

### Running unit tests

Run `nx test color-variants` to execute the unit tests via [Vitest](https://vitest.dev/).
