...
# Razorwind - Ghostty Theme Generator

**Razorwind - Ghostty Theme Generator** creates [Ghostty](https://ghostty.org/) terminal theme files from design tokens. Color layout follows [Dracula for Ghostty](https://draculatheme.com/ghostty) (`palette`, `background`, `foreground`, cursor, selection).
...
```bash
npm install -D @razorwind/ghostty
```
      ...
      mapTheme: tokens => {
        ...
        return {
          ...
          palette: {
            ...
            3: color("color.ansi.yellow"),
            4: color("color.ansi.blue"),
            5: color("color.ansi.magenta"),
            6: color("color.ansi.cyan"),
            7: color("color.ansi.white")
          }
        };
      }
    })
...
Generated files (under `ghostty-themes/` by default):

- `{theme-name}` — Ghostty theme config (no file extension) from `mapTheme`
- `INSTALL.md` — copy into `~/.config/ghostty/themes/`, set `theme =`, reload

Activate like [Dracula Ghostty](https://draculatheme.com/ghostty):

```sh
mkdir -p ~/.config/ghostty/themes
cp ghostty-themes/my-theme ~/.config/ghostty/themes/
...
| `outputPath`   | `"ghostty-themes"` | Output directory for theme files                  |
| `installGuide` | _(generated)_      | Override body for generated `INSTALL.md`          |

`mapTheme` return shapes: a single `GhosttyTheme`, an array, or a `Record<string, GhosttyTheme>`. Each theme needs a `name`. Map token colors into `palette` (0–15), `background`, `foreground`, `cursorColor`, `cursorText`, `selectionForeground`, `selectionBackground`, or pass extra Ghostty options via `config`.

## Development

### Building

Run `nx build ghostty` to build the library.
