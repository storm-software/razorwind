...
# Razorwind - Sandpack Theme Generator

**Razorwind - Sandpack Theme Generator** creates [Sandpack](https://sandpack.codesandbox.io/) custom themes from design tokens and emits component `usage` demos as Sandpack [`files`](https://sandpack.codesandbox.io/docs/getting-started/usage#files) payloads for docs / Storybook.
...
```bash
npm install -D @razorwind/sandpack
```
      ...
      mapTheme: tokens => {
        ...
        return {
          ...
          colors: {
            ...
            clickable: color("color.muted")
          },
          syntax: {
            plain: color("color.fg"),
            comment: { color: color("color.muted"), fontStyle: "italic" },
...
Generated files (under `sandpack/` by default):

- `themes/{theme-name}.json` — Sandpack `theme` prop objects from `mapTheme`
- `usage/{component}/{demo}.json` — Sandpack `files` demos from `components[].usage`
- `INSTALL.md` — import theme / files into `<Sandpack />`

Load a theme and usage demo:

```tsx
import { Sandpack } from "@codesandbox/sandpack-react";
import theme from "./sandpack/themes/my-theme.json";
import demo from "./sandpack/usage/button/button-default.json";
...
See [Sandpack custom themes](https://sandpack.codesandbox.io/docs/getting-started/themes#custom-theme) and [files](https://sandpack.codesandbox.io/docs/getting-started/usage#files).
...
| `mapFiles`     | _(from usage)_ | Optional `(components, tokens) =>` usage demos                    |
| `includeUsage` | `true`        | Emit `usage/` demos from `schema.components[].usage`              |
| `template`     | `"react"`     | Default Sandpack template for auto-built usage demos              |
| `outputPath`   | `"sandpack"`  | Output directory for theme / usage files                          |
| `installGuide` | _(generated)_ | Override body for generated `INSTALL.md`                          |

`mapTheme` return shapes: a single `SandpackTheme`, an array, or a `Record<string, SandpackTheme>`. Each theme needs a `name`. Map token colors into `colors`, `syntax`, and optional `font` (Sandpack accepts partial themes).

## Development

### Building

Run `nx build sandpack` to build the library.

### Running unit tests

Run `nx test sandpack` to execute the unit tests via [Vitest](https://vitest.dev/).
