...
# Razorwind - Tamagui Generator

**Razorwind - Tamagui Generator** is a Power Plant generator that creates Tamagui theme tokens from Razorwind design tokens.
...
```bash
npm install -D @razorwind/tamagui
```
...
Run `nx test tamagui` to execute the unit tests via [Vitest](https://vitest.dev/).
