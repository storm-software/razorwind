...
# Razorwind - Notepad++ Theme Generator

**Razorwind - Notepad++ Theme Generator** creates [Notepad++](https://notepad-plus-plus.org/) `*.xml` theme files from design tokens. Structure follows [Dracula for Notepad++](https://draculatheme.com/notepad-plus-plus) (`LexerStyles`, `GlobalStyles`, `WordsStyle`, `WidgetStyle`).
...
```bash
pnpm add -D @razorwind/notepad-plus-plus
```
...
```bash
npm install -D @razorwind/notepad-plus-plus
```
...
```bash
yarn add -D @razorwind/notepad-plus-plus
```
...
Generated files (under `notepad-plus-plus-themes/` by default):

- `{theme-name}.xml` — Notepad++ theme XML from `mapTheme`
- `INSTALL.md` — copy into `%AppData%\Notepad++\themes`, activate in Style Configurator

Activate like [Dracula Notepad++](https://draculatheme.com/notepad-plus-plus):

1. Copy `notepad-plus-plus-themes/my-theme.xml` to `%AppData%\Notepad++\themes\`
2. Restart Notepad++
3. **Settings → Style Configurator → Theme** → select **my-theme** → **Save & Close**
...
| `outputPath`   | `"notepad-plus-plus-themes"` | Output directory for theme files                  |
| `installGuide` | _(generated)_                | Override body for generated `INSTALL.md`          |

`mapTheme` return shapes: a single `NotepadPlusPlusTheme`, an array, or a `Record<string, NotepadPlusPlusTheme>`. Each theme needs a `name` plus `lexerStyles`, `globalStyles`, `body`, or `xml`. Colors accept `#rrggbb` or `rrggbb` and are written as Notepad++ `RRGGBB` values.

## Development

### Building

Run `nx build notepad-plus-plus` to build the library.

### Running unit tests

Run `nx test notepad-plus-plus` to execute the unit tests via [Vitest](https://vitest.dev/).
