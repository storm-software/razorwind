...
# Razorwind - AI Documentation Generator

**Razorwind - AI Documentation Generator** creates an
[llms.txt](https://llmstxt.org/) index and focused design-system companion
documents from a Razorwind schema. The output gives AI agents concise,
structured references for approved tokens, components, icons, and fonts.
...
export default defineConfig({
  plugins: [
    llms({
      ...
      summary: "Components and foundations for Acme products.",
...
The plugin generates these files in the Razorwind output directory:

- `llms.txt` — discovery index with links to the focused references
- `llms-tokens.txt` — token paths, values, themes, and descriptions
- `llms-components.txt` — component metadata, dependencies, files, and usage
- `llms-icons.txt` — icon metadata, aliases, tags, and themed asset paths
- `llms-fonts.txt` — Google and local font configuration

Companion links are relative by default. Set `baseUrl` when the files will be
published at a known absolute URL:

```ts
llms({
  baseUrl: "https://design.example.com/ai/",
  outputPath: "public/ai"
});
```
...
| --- | --- | --- |
| `outputPath` | generation directory | Directory containing all five files |
| `title` | schema title or name | H1 used in the index |
| `summary` | schema description | Short blockquote below the H1 |
| `details` | omitted | Additional Markdown before the link sections |
| `baseUrl` | omitted | Absolute HTTP(S) base URL for companion links |

The `details` value may contain Markdown, but not H1 or H2 headings because
those levels structure the llms.txt index.

### Rendering without the plugin

Renderers are also exported for tools that need the Markdown in memory:
...
Run `pnpm nx test llms` for unit tests, `pnpm nx build llms` to build the
package, and `pnpm nx typecheck llms` to check its TypeScript contracts.
