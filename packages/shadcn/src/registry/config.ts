...
const COMPONENTS_CONFIG_LOAD_OPTIONS = {
  configFile: COMPONENTS_CONFIG_FILE,
  dotenv: false,
  envName: false,
  packageJson: false,
  rcFile: false,
...
async function loadComponentsConfig(cwd: string) {
  let currentDir = cwd;

  while (true) {
    const result = await loadConfig({
      cwd: currentDir,
      ...COMPONENTS_CONFIG_LOAD_OPTIONS
    });
...
export type RegistryConfig = ShadcnConfig;

export async function getRegistryConfig(cwd: string) {
...
export async function getRawConfig(
  ...
  try {
    const configResult = await loadComponentsConfig(cwd);
    ...
    const componentPath = configFile ?? `${cwd}/${COMPONENTS_CONFIG_FILE}`;
...
export async function getWorkspaceConfig(config: RegistryConfig) {
  ...
  for (const key of Object.keys(config.aliases)) {
    ...
    const workspaceConfig = await getRegistryConfig(packageRoot);
...
): key is Exclude<Extract<keyof RegistryConfig["aliases"], string>, "utils"> {
...
export function createRegistryConfig(
  partial?: DeepPartial<RegistryConfig>
): RegistryConfig {
