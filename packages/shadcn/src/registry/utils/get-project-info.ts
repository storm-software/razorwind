...
import { getRegistryConfig, resolveConfigPaths } from "../config";
import type { ShadcnRawConfig } from "../shadcn-types";
...
export async function getProjectInfo(
  ...
  const isUsingAppDir = existsSync(
    joinPaths(cwd, `${isSrcDir ? "src/" : ""}app`)
  );
  ...
  if (appConfig?.length) {
    const appConfigContents = await readFile(joinPaths(cwd, appConfig));
...
export async function getTailwindCssFile(cwd: string, configCssFile?: string) {
  ...
  for (const file of files) {
    const contents = await readFile(joinPaths(cwd, file));
    if (
...
export async function getTsConfigAliasPrefix(
  cwd: string
): Promise<string | null> {
  const tsConfig = await getTsConfig(cwd);
  const paths = Object.entries(tsConfig?.compilerOptions.paths ?? {}).length
...
export async function getProjectAliasInfo(
  cwd: string
): Promise<{ prefix: string | null; source: string | null }> {
  const tsConfigAliasPrefix = await getTsConfigAliasPrefix(cwd);
  const packageImportPrefix = await getPackageImportPrefix(cwd);
...
export async function getProjectConfig(
  ...
  const packageImportAliases =
    aliasInfo.source === "package_imports"
      ? await getPackageImportAliases(cwd)
      : null;
...
export async function getProjectTailwindVersionFromConfig(config: {
  resolvedPaths: Pick<RegistryConfig, "resolvedPaths">["resolvedPaths"];
...
export async function getProjectComponents(cwd: string) {
  const existingConfig = await getRegistryConfig(cwd);
  if (!existingConfig) {
    return [];
  }
