...
# Razorwind - Chrome Theme Generator

**Razorwind - Chrome Theme Generator** creates [Google Chrome](https://www.google.com/chrome/) extension theme folders from design tokens. Output matches community themes such as [Dracula for Chrome](https://github.com/dracula/google-chrome) (`manifest.json` plus optional images and icons).
...
```bash
npm install -D @razorwind/chrome
```
      ...
      mapTheme: tokens => {
        ...
        return {
          ...
          colors: {
            ...
            omnibox_background: rgb("color.bg"),
...
Generated files (under `chrome-themes/` by default):

- `{theme-slug}/manifest.json` — Chrome extension theme manifest from `mapTheme`
- `INSTALL.md` — load each folder as an unpacked extension at `chrome://extensions`

Install like [Dracula for Chrome](https://github.com/dracula/google-chrome):

1. Open `chrome://extensions`
2. Enable **Developer mode**
3. Click **Load unpacked** and select `chrome-themes/my-chrome-theme/`

Add any `images/` or icon PNGs referenced in `manifest.json` before loading.
...
| `outputPath`   | `"chrome-themes"` | Output directory for theme folders                |
| `installGuide` | _(generated)_     | Override body for generated `INSTALL.md`          |

`mapTheme` return shapes: a single `ChromeTheme`, an array, or a `Record<string, ChromeTheme>`. Each theme needs `name` and `colors`. Color values may be hex strings (`#rrggbb`) or `[r, g, b]` tuples (0–255). Optional fields mirror Chrome's [theme manifest](https://developer.chrome.com/docs/extensions/develop/ui/themes): `images`, `icons`, `tints`, and `properties`.

## Development

### Building

Run `nx build chrome` to build the library.

### Running unit tests

Run `nx test chrome` to execute the unit tests via [Vitest](https://vitest.dev/).
