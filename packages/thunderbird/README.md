...
# Razorwind - Thunderbird Theme Generator

**Razorwind - Thunderbird Theme Generator** creates [Mozilla Thunderbird](https://www.thunderbird.net/) extension theme folders from design tokens. Output matches community themes such as [Dracula for Thunderbird](https://draculatheme.com/thunderbird) (`manifest.json` plus optional icons).
...
```bash
pnpm add -D @razorwind/thunderbird
```
...
```bash
npm install -D @razorwind/thunderbird
```
...
```bash
yarn add -D @razorwind/thunderbird
```
...
Generated files (under `thunderbird-themes/` by default):

- `{theme-slug}/manifest.json` — Thunderbird extension theme manifest from `mapTheme`
- `INSTALL.md` — zip each folder and install via Add-ons and Themes

Install like [Dracula for Thunderbird](https://draculatheme.com/thunderbird):

1. Zip a generated theme folder so `manifest.json` is at the archive root
2. Open Thunderbird → **Settings** → **Add-ons and Themes**
3. Gear menu → **Install Add-on From File…** and select the ZIP

Add any icon PNGs referenced in `manifest.json` before installing.
...
| `outputPath`   | `"thunderbird-themes"` | Output directory for theme folders           |
| `installGuide` | _(generated)_     | Override body for generated `INSTALL.md`          |

`mapTheme` return shapes: a single `ThunderbirdTheme`, an array, or a `Record<string, ThunderbirdTheme>`. Each theme needs `name`, `gecko.id`, and `colors`. Color values may be hex strings (`#rrggbb`) or `[r, g, b]` tuples (0–255); they are rendered as `rgb(r, g, b)` strings per the [Dracula Thunderbird manifest](https://github.com/dracula/thunderbird/blob/master/manifest.json). Optional fields include `icons` and `gecko.strictMinVersion`.

## Development

### Building

Run `nx build thunderbird` to build the library.

### Running unit tests

Run `nx test thunderbird` to execute the unit tests via [Vitest](https://vitest.dev/).
