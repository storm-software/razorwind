...
# Razorwind - Vivaldi Theme Generator

**Razorwind - Vivaldi Theme Generator** creates [Vivaldi](https://vivaldi.com/) browser theme folders from design tokens. Output matches shareable Vivaldi themes such as [Dracula for Vivaldi](https://draculatheme.com/vivaldi) (`settings.json` plus optional background image).
...
```bash
npm install -D @razorwind/vivaldi
```
...
Generated files (under `vivaldi-themes/` by default):

- `{theme-slug}/settings.json` — Vivaldi theme document from `mapTheme`
- `INSTALL.md` — zip each folder and import via **Settings → Themes → Open Theme…**

Install like [Dracula for Vivaldi](https://draculatheme.com/vivaldi):

```sh
cd vivaldi-themes/my-theme
zip -r ../my-theme.zip .
# Vivaldi → Settings → Themes → Open Theme… → select my-theme.zip
```
...
| `outputPath`   | `"vivaldi-themes"` | Output directory for theme folders                |
| `installGuide` | _(generated)_      | Override body for generated `INSTALL.md`          |

`mapTheme` return shapes: a single `VivaldiTheme`, an array, or a `Record<string, VivaldiTheme>`. Each theme needs `name`, `colorBg`, and `colorFg`. Optional fields mirror Vivaldi's `settings.json`: accent/highlight/window colors, transparency, blur, radius, `backgroundImage`, and a stable `id` UUID for published themes.

## Development

### Building

Run `nx build vivaldi` to build the library.

### Running unit tests

Run `nx test vivaldi` to execute the unit tests via [Vitest](https://vitest.dev/).
