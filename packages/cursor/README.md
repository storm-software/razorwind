...
# Razorwind - Cursor Theme Extension Plugin

**Razorwind - Cursor Theme Extension** creates Cursor-installable theme extensions from design tokens. Cursor is a VS Code fork and loads the same VSIX theme format ([Dracula Cursor](https://draculatheme.com/cursor) is a good reference).
...
```bash
npm install -D @razorwind/cursor
```
...
Generated package (under `cursor-extension/` by default):

- `themes/*.json` — VS Code–compatible theme documents from `mapTheme`
- `package.json` — extension manifest + `contributes.themes` + `package-vsix` script
- `.vscodeignore`, `README.md`
- `INSTALL.md` — Cursor VSIX install steps
- `scripts/` — VSIX packaging helpers (`buildCursorPackage`, `vsixPackageShim`)

### Options

| Option           | Default               | Description                                                       |
| ---------------- | --------------------- | ----------------------------------------------------------------- |
| `name`           | _(required)_          | Unscoped extension id                                             |
| `publisher`      | _(required)_          | Extension publisher id                                            |
| `mapTheme`       | _(required)_          | `(tokens) =>` theme, theme array, or theme record                 |
| `outputPath`     | `"cursor-extension"`  | Output directory for the extension package                        |
| `displayName`    | title-cased `name`    | Extension / Color Theme display name                              |
| `version`        | `"0.0.1"`             | Extension version (no prerelease tags)                            |
| `includeScripts` | `true`                | Emit packaging `scripts/` + `package.json` script entries         |
| `extensionName`  | `name`                | Unscoped name used by the VSIX shim / `dist/<name>.vsix`          |
| `installGuide`   | _(generated)_         | Override body for generated `INSTALL.md`                          |

`mapTheme` return shapes: a single `CursorTheme`, an array, or a `Record<string, CursorTheme>`. Theme `type` is `light` | `dark` | `hc` | `hcLight` (maps to `uiTheme` in `contributes.themes`).

### Install in Cursor

After generation, from the output directory:

```bash
pnpm package-vsix
```

Then in Cursor: Command Palette → **Extensions: Install from VSIX...** → select `./dist/<name>.vsix` → **Preferences: Color Theme**. See generated `INSTALL.md` for the full steps.

## Development

### Building

Run `nx build cursor` to build the library.
...
Run `nx test vsce` to execute the unit tests via [Vitest](https://vitest.dev/).
