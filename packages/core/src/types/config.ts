...
export interface Options {
   ...
   *
   * Files that match are grouped under that theme key. Non-theme files are merged into a shared `base` entry when at least two distinct themes are detected, and are also included when resolving each theme so references into shared primitives succeed. The shared `base` set (and color-variant expansions such as `baseDimmed`) is omitted from generated output. If fewer than two themes are found, all sources are merged into a single {@link Tokens} object instead of a record.
   *
